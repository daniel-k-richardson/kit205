/* File name: main.c
* Purpose: This is the driver (or harness) file for 205 Assignment 1 Part. B This
* file handles how the user interacts with courselist, course and AVL.
*
...
{
	while (self != NULL) {
		if (search_AVL(self->data.students, student_id)) {
			printf("%s\n", self->data.name);
		}
		self = self->next;
	}
...
void enrol_student(CourseList *self, char *course, long student_id)
{
	...
	temp->data.students = add_element_to_AVL(temp->data.students, student_id);
}
...
void unenrol_student(CourseList *self, char *course, long student_id)
{
	...
	tmp->data.students = delete_element_from_AVL(tmp->data.students, student_id);
}
...
void course_summary(CourseList self)
{
	...
	while (self != NULL) {
		number_of_students = number_of_elements_in_AVL(self->data.students);
